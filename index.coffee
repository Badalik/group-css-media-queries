...
# parse it
  ...
  for media, rules of medias
    ...
    if media.indexOf("max-width") isnt -1
      ...
      rule.unit = m[2] if m[2]
    # extract min-height and max-height values
    if media.indexOf("min-height") isnt -1
      m = media.match ///min-height:\s*(\d+)(px|em)?///
      rule.minHeight = parseInt m[1] if m && m[1]
      rule.unit = m[2] if m[2]
    if media.indexOf("max-height") isnt -1
      m = media.match ///max-height:\s*(\d+)(px|em)?///
      rule.maxHeight = parseInt m[1] if m && m[1]
      rule.unit = m[2] if m[2]
    mediaRules.push rule

  # break rules into only-min, only-max, intervals and others
  onlyMinRules = mediaRules.filter (rule) ->
    rule.minWidth? and not rule.maxWidth? and not rule.minHeight? and not rule.maxHeight?
  onlyMaxRules = mediaRules.filter (rule) ->
    rule.maxWidth? and not rule.minWidth? and not rule.minHeight? and not rule.maxHeight?
  intervalRules = mediaRules.filter (rule) ->
    rule.minWidth? and rule.maxWidth? and not rule.minHeight? and not rule.maxHeight?
  combinedRules = mediaRules.filter (rule) ->
    rule.minHeight? or rule.maxHeight?
